<?php
    ...
    $resTotal = $conn->query("SELECT id_categoria, nombre_categoria FROM categorias");
    $categoria = array();
    if ($resTotal) {
        while ($row = $resTotal->fetch_assoc()) {
            $categoria[] = array('name' => $row['nombre_categoria'], 'id' => $row['id_categoria']);
        }
    } else {
        echo "Error en la consulta: " . $conn->error;
    }

    $resTotal2 = $conn->query("SELECT id_proveedor, nombre FROM proveedores");
    $proveedor = array();
    if ($resTotal2) {
        while ($row = $resTotal2->fetch_assoc()) {
            $proveedor[] = array('name' => $row['nombre'], 'id' => $row['id_proveedor']);
        }
    } else {
        echo "Error en la consulta: " . $conn->error;
    }
...
<div class="container" style="margin: auto;">
    ...
    <form id="productForm" method="post" action="actualizar_producto.php" style="display: none;">
    <div>
        <label for="idProd">ID Producto:</label>
        <input type="text" id="idProd" name="idProd" required pattern="[0-9]+" title="Ingrese solo números" required disabled>
    </div>
    <div>
...
$(document).ready(function() {
    ...
    $('#showFormBtn').on('click', function() {
        ...
        $('#idProd').prop('readonly', false); // Asegúrate de que ID no esté en modo solo lectura
    });
    ...
    $('#submitBtn').on('click', function(e) {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                Swal.fire({
                    title: 'Error',
                    text: 'Hubo un error al agregar el producto: ' + textStatus + ' - ' + errorThrown,
                    icon: 'error',
                    confirmButtonText: 'OK',
                    customClass: {
                        popup: 'custom-swal-popup'
...
$('#btnExportar').on('click', function() {
    ...
    var filename = 'Productos_' + formattedDate + '.xlsx';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $('#btnExportarPDF').on('click', function() {
        ...
        var filename = 'Productos_' + formattedDate + '.pdf';
        doc.save(filename);
    });
});
