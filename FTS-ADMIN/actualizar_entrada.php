...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id_entrada'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM entradas WHERE id_entrada = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id_proveedor = $_POST['id_proveedor']; // Aquí utilizamos 'proveedor' como 'id_proveedor'

    try {
        // Preparar la consulta SQL UPDATE para actualizar los datos de la entrada
        $stmt = $pdo->prepare("UPDATE entradas SET fecha = :fecha, id_producto = :id_producto, cantidad = :cantidad, id_proveedor = :id_proveedor WHERE id_entrada = :id");
